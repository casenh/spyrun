...
if __name__ == '__main__':
	...
	json_dump_file = open("patientJSON.txt", 'w')
	random.seed(100)
	for n in range(0,1000):
		...
		#Create the JSON
		json_dump = json.dumps([{'username': username, 'accountPrimary': (True), 'patientGender': (gender), 'patientBirthdate': (month + "/" + day + "/" + year), 'patientZip': (addressZip), 'patientState': (state), 'patientAddress': (address), 'patientName': (firstName + " " + lastName), 'patientID': (str(n))}])
		json_dump_file.write(json_dump[1:-1] + '\n')
	json_dump_file.close()
	subprocess.call(['mongoimport', '--db', 'entities', '--collection', 'patients', '--type', 'json', '--file', './patientJSON.txt'])
